...
6. **Add a Blob store** so file uploads (GPS/video/session animation/PDF, logos) actually work: in the Vercel project, go to **Storage → Create Database → Blob**, and connect it to the project — see "File storage" below. This isn't optional on Vercel: the deployed app's filesystem is read-only, so any upload attempted before a Blob store is connected fails outright (a logo/GPS/video upload throws an error) rather than silently degrading.
...
- **File storage**: `src/lib/storage.ts` writes to local disk when running locally (fine for dev — `process.env.VERCEL` is unset). Any time it detects it's actually running on Vercel, it always uploads to Vercel Blob instead, regardless of exactly which env vars a connected store happens to expose — newer Blob stores authenticate automatically at runtime rather than via a static `BLOB_READ_WRITE_TOKEN`, so the app no longer gates on that variable's presence. Either way, files are only ever served through the access-controlled routes (`/api/files/[assetId]`, `/api/org-logo/[orgId]`), which check organization membership server-side before streaming — the Blob URL itself is never exposed to the client.
